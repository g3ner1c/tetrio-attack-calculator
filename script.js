...
const ATTACK_DICT = {
    // base settings for each attack type
...
function attackFunc(combo_level, base_attack, B2B_level) {
    ...
    if (B2B_level + base_attack == 0) {
        ...
        return (
            Math.floor(combo_level / (4 / (B2B_level + base_attack))) +
            B2B_level +
            base_attack
        );
    }
...
function updateFoot() {
    document.getElementById("AttackFoot").innerHTML =
        "<tr> <td></td>" +
        "<td> <b> <i>Total:</i></b> </td>" +
        "<td> <b> <i>" +
        total_lines +
...
function updateTable() {
    ...
    for (var attack of attack_list) {
        attackHTML +=
            ...
            attack["num"] + // #
            "</td> <td>" +
            ATTACK_DICT[attack["attack_id"]][0] + // Attack type
            "</td> <td> <b>" +
            (attack["lines_sent"] + Number(attack["PC"]) * 10) + // Lines Sent
            "</b> </td> <td>" +
            attack["combo"] + // Combo
            "</td> <td>" +
            attack["B2B_count"] + // B2B Count
            "</td> <td>" +
            b2bCountToLevel(attack["B2B_count"]) + // B2B Level
            "</td>";
...
function buttonAdd() {
    ...
    if (ATTACK_DICT[attack_selected][2] && B2Bing) {
        // if attack is B2B-able and B2B is turned on

        B2B_count++;
    } else if (ATTACK_DICT[attack_selected][2]) {
        // if attack is B2B-able and B2B is turned off

        B2B_count = 0;
    } else {
        // attack is not B2B-able
    ...
    attack_list.push({
        ...
        combo: combo_level,
        B2B_count: B2B_count,
        lines_sent: attackFunc(
            combo_level,
            ATTACK_DICT[attack_selected][1],
            b2bCountToLevel(B2B_count)
        ),
        PC: false
    });
...
function toggleCombo() {
    if (comboing) {
        ...
        document.getElementById("toggle_combo").innerHTML =
            "Comboing <b> Off </b>";
    } else {
        comboing = true;
